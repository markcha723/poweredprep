const parseCreateRequest = (incomingRequest: any) => {
  const {
    section,
    questionTypes,
    passageTopics,
  ...
  let chatCompletionsObject: Object;
  let systemMessage =
  ...
  let textForNumberOfQuestions = "";
  let textForDifficulty = "";
  let textForWordsToUse = "";

  // each of these strings, when set, should have a space at the end to accomodate following text.

  // assigning section text.
  switch (section) {
    case "reading":
      textForSection =
  ...
  // assigning textForNumberOfQuestions
  textForNumberOfQuestions = `Generate ${numberOfQuestions} questions. `;

  // assigning textForDifficulty
  ...
  chatCompletionsObject = {
    systemMessage,
    prompt: `${textForSection}${textForQuestionTypes}${textForPassageTopics}${textForPassageStyles}${textForDifficulty}${textForDifficulty}${textForWordsToUse}`,
  };

  return chatCompletionsObject;
};
